...
class SwitchInfo:
    ...
    def load_json_str(self, json_str, architecture_spec=None):
        ...
        for res_type, res_dict in [
            (ResType.table, self.tables),
            (ResType.action_prof, self.action_profs),
            (ResType.action, self.actions),
            (ResType.meter_array, self.meter_arrays),
...
def BmMatchParam_to_str(self):
    return (
        BmMatchParamType._VALUES_TO_NAMES[self.type]
        + "-"
...
class ThriftAPI:
    ...
    def table_modify_match(self, table_name, action_name, match_keys, action_params=[]):
        ...
        if entry_handle is not None:
            ...
            raise UIn_Error("Table {} has no match {}".format(table_name, match_keys))
        return entry_handle
