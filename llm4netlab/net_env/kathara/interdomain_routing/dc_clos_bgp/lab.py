...
class DCClosBGP(NetworkEnvBase):
    ...
    def __init__(self):
        ...
        for pod in range(SUPER_SPINE_COUNT):
            ss_router_meta = tot_super_spines[pod]
            for spine_router_meta in tot_spines:
                self.lab.connect_machine_to_link(
                    ss_router_meta.machine.name, f"{ss_router_meta.machine.name}_{spine_router_meta.machine.name}"
                )
                self.lab.connect_machine_to_link(
                    spine_router_meta.machine.name, f"{ss_router_meta.machine.name}_{spine_router_meta.machine.name}"
                )
                subnet = subnets31.pop(0)
                a_ip, b_ip = assign_p2p_ips(subnet)
                ss_router_meta.cmd_list.append(f"ip addr add {a_ip} dev eth{ss_router_meta.eth_index}")
                ss_router_meta.eth_index += 1
                spine_router_meta.cmd_list.append(f"ip addr add {b_ip} dev eth{spine_router_meta.eth_index}")
                spine_router_meta.eth_index += 1

                # add BGP neighbor config
                ss_router_meta.frr_neighbor_configs.append(
                    FRR_NEIGHBOR_ADD_TEMPLATE.format(
                        neighbor_ip=b_ip.split("/")[0],
                        neighbor_as=spine_router_meta.AS_number,
                    )
                )
                spine_router_meta.frr_neighbor_configs.append(
                    FRR_NEIGHBOR_ADD_TEMPLATE.format(
                        neighbor_ip=a_ip.split("/")[0],
                        neighbor_as=ss_router_meta.AS_number,
                    )
                )

                # add router ID
                if ss_router_meta.router_id == "":
                    ss_router_meta.router_id = a_ip.split("/")[0]
                if spine_router_meta.router_id == "":
                    spine_router_meta.router_id = b_ip.split("/")[0]
        ...
        for pod in range(SUPER_SPINE_COUNT):
            for idx, (leaf_meta, host) in enumerate(zip(pod_leaves[pod], pod_hosts[pod])):
                self.lab.connect_machine_to_link(
        ...
        for router_meta in tot_super_spines + tot_spines:
            ...
            router_meta.machine.create_file_from_path(
                os.path.join(BASE_DIR, "utils/kathara/bgp_utils/daemons"), "/etc/frr/daemons"
            )
            router_meta.machine.create_file_from_path(
                os.path.join(BASE_DIR, "utils/kathara/bgp_utils/vtysh.conf"), "/etc/frr/vtysh.conf"
            )
